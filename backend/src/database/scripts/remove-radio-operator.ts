...
async function removeRadioOperator() {
  ...
  try {
    ...
    console.log(`Found ${soldiersWithRadioOperator.length} soldiers with radio_operator`);
    ...
    for (const soldier of soldiersWithRadioOperator) {
      ...
      const hasRadioOperator = rolesArray.some(r => r.toLowerCase() === 'radio_operator');
      
      if (hasRadioOperator) {
        const newRoles = rolesArray.filter(role => role.toLowerCase() !== 'radio_operator');
        ...
        updatedCount++;
        console.log(`Updated ${soldier.name}: ${rolesArray.join(', ')} -> ${soldier.roles.join(', ')}`);
      }
    }
    ...
    for (const soldier of allSoldiers) {
      ...
      const hasRadioOperator = rolesArray.some(r => r.toLowerCase() === 'radio_operator');
      if (hasRadioOperator && !soldiersWithRadioOperator.find(s => s.id === soldier.id)) {
        const newRoles = rolesArray.filter(role => role.toLowerCase() !== 'radio_operator');
        if (newRoles.length === 0) {
          newRoles.push('soldier');
        }
