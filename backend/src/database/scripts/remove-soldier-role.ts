...
async function removeSoldierRole() {
  ...
  try {
    ...
    console.log(
      `Found ${soldiersWithSoldierRole.length} soldiers with 'soldier' role`,
    );
    ...
    for (const soldier of soldiersWithSoldierRole) {
      ...
      const hasSoldierRole = rolesArray.some(
        (r) => r.toLowerCase() === 'soldier',
      );

      if (hasSoldierRole) {
        // Remove 'soldier' role - everyone is a soldier by default
        const newRoles = rolesArray.filter(
          (role) => role.toLowerCase() !== 'soldier',
        );
        ...
        const after =
          newRoles.length > 0 ? newRoles.join(', ') : '(ללא תפקידים מיוחדים)';
    ...
    console.log(
      'Note: Everyone is a soldier by default, so this role is no longer needed.',
    );
  } catch (error) {
