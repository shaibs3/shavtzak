...
export function TaskForm({ task, onSubmit, onCancel }: TaskFormProps) {
  ...
  const allRoles = useMemo(() => {
    ...
    // Handle both null, undefined, string (from TypeORM simple-array), and array cases
    let rolesArray: string[] = [];
    if (customRoles === null || customRoles === undefined) {
      rolesArray = [];
    } else if (typeof customRoles === 'string') {
      // TypeORM simple-array might return as comma-separated string
      rolesArray = customRoles.split(',').filter(r => r.trim().length > 0);
    } else if (Array.isArray(customRoles)) {
      rolesArray = customRoles;
    }
    
    const result = getAllRoles(rolesArray);
    // Debug log - remove in production
    console.log('TaskForm - Settings:', settings, 'CustomRoles:', customRoles, 'AllRoles:', result);
    return result;
  }, [settings]);
